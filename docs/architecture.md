...
## 84. Forensic Export (CURRENT IMPLEMENTATION)

`python scripts/export_run.py <run_id>` produces one self-contained JSON
(or JSON.gz) artifact for a single run, designed to be uploaded directly
to an AI system (Claude, Copilot, or a future forensic tool) for incident
investigation -- see the database-architecture and AI-usability reviews
this implements. Read-only, zero side effects, and every interpretive
field is a deterministic rollup of stored data -- nothing in the artifact
is AI-generated, inferred, or scored.

### AI-oriented design rationale

The stated goal was "an AI should understand what happened within seconds
without reconstructing relationships across multiple sections." Three
design choices follow directly from that:

1. **Pre-correlated, not raw.** `SAFETY_FAULT_RAISED`/`ACKNOWLEDGED`
   events are joined by `fault_id` into one `safety.fault_timeline` entry
   per fault -- the AI is never asked to match two lists itself. Same for
   `timeline`, which merges `event_log` + `station_state` transitions
   into one chronologically-sorted list instead of leaving three
   differently-shaped, differently-timestamped tables for the reader to
   interleave.
2. **Interpretation kept separate from, but traceable to, raw fact.**
   `incident_summary` (the 30-second digest) and `forensic_findings` (the
   deterministic confidence/communication-failure mapping) are both
   generated by fixed Python logic from `safety`/`run_summary` -- never
   duplicated derivation logic, never a second source of truth for the
   same fact.
3. **Never let "unavailable" collapse into "confirmed clean."** Every
   telemetry-dependent section (`safety`, `measurements`,
   `raw_hardware_log`) uses `available: false` + an explicit `reason`
   when the telemetry database can't be reached -- fault counts become
   `None` (unknown), never `0` (which would falsely read as "no fault
   occurred"). This matters specifically because `event_log` -- the only
   source of safety-fault detail -- lives in the *telemetry* database
   (Section 83), not the always-available index database.

### Read-only export path

`DataStorage.open_for_forensic_export(settings, run_id)` (classmethod) is
the one new entry point everything else builds on:

- Opens the index database via `sqlite3`'s `file:...?mode=ro` URI --
  **never allocates a `run_sequence` value, never runs `CREATE TABLE`/
  migration, never creates a CSV directory.** Both properties are
  verified directly (`tests/test_export_run.py::ReadOnlyExportPathTests`)
  by checking `run_sequence.next_value` and the CSV directory's existence
  are unchanged after an export.
- Looks up `run_id` via the *existing* `get_run_summary()` -- reused
  verbatim, no new query logic.
- Resolves telemetry **only** via the looked-up `run_summary.telemetry_db`
  column (never inferred from a timestamp, never guessed at a filename)
  and opens that exact file, also read-only. If `telemetry_db` is `NULL`
  (a pre-Phase-B legacy run) or the named file no longer exists, returns
  the instance anyway with `self.telemetry_unavailable_reason` set and
  `self._db` left `None` -- every index-database-backed section (
  `run_summary`, `execution_state`, and the parts of `incident_summary`
  sourced from them) is still exported in full; only the telemetry-
  dependent sections degrade, explicitly.
- Returns `(None, "reason")` only when `run_id` itself can't be found at
  all (or the index database doesn't exist) -- there is genuinely nothing
  to export in that case, unlike the partial-telemetry case above.

Two new read methods were added to `DataStorage`, and one existing method
extended, all reused directly by the exporter with no duplicated SQL:

- `get_station_state_for_run(run_id)` -- new. `get_last_execution_state()`
  deliberately reads *across* every run_id (by design, for "what did the
  station last do"); a forensic export needs exactly one run's own
  transitions.
- `get_raw_hardware_log(run_id)` -- new. `DataStorage` never *writes* this
  table (`RawHardwareLogWriter` owns that via its own connection -- see
  Section 41/79), but it is a normal table in the telemetry file
  `DataStorage` already has open, so reading it needed no new connection,
  only a new query method.
- `get_recent_events(limit=None)` -- extended (previously always applied
  a `LIMIT`, defaulting to 20). `limit=None` returns the complete event
  narrative for a run. Every existing caller passes an explicit int and
  is unaffected.

### Section-by-section structure

Top-level key order (json.dump() preserves dict insertion order --
`sort_keys=True` is never used, that would alphabetize and destroy this):
...
- **`metadata`** -- `export_version`, `exported_at`, `run_id`,
  `telemetry_db`, `telemetry_available`/`telemetry_unavailable_reason`,
  `generator`, `flags`, `sections_included`, and a fixed `description`
  string explaining the artifact's own structure to a reader (human or
  AI) that has never seen this schema before.
- **`incident_summary`** -- the 30-second digest: `result`,
  `stop_reason`, `duration_s`, `final_execution_state` (the run's last
  `station_state.state` -- an `ACTIVE` final state that never reached a
  terminal state is itself a red flag), `safety_fault_count`,
  `acknowledged_fault_count`, `operator_acknowledged`,
  `testing_continued_after_fault`, `primary_failure_mode`,
  `primary_device`, `additional_fault_count`, `root_cause_caveat`, and a
  template-generated `narrative` sentence. Uses **`primary_failure_mode`**,
  not "root cause" -- `verification_result` describes the failure mode
  observed (the SMU readback's own report), not a diagnosis of the
  underlying physical cause; `root_cause_caveat` states this explicitly
  whenever a fault exists, and no field anywhere in the export uses the
  term "root cause".
- **`timeline`** -- `event_log` + `station_state` transitions merged and
  sorted by timestamp; each entry is `{timestamp, source, event, detail,
  severity}`. Deliberately excludes `raw_hardware_log` (too high-volume,
  and its safety-relevant rows are already represented via `event_log`'s
  `SAFETY_FAULT_RAISED` entries).
- **`safety`** -- `fault_timeline`, each entry pre-joining a
  `SAFETY_FAULT_RAISED` event with its `SAFETY_FAULT_ACKNOWLEDGED`
  counterpart (matched by `fault_id`) and carrying `device_name`/
  `device_type`/`position`/`context`/`source_method`/`reason`/
  `verification_result`/`verification_result_meaning`.
- **`forensic_findings`** -- one entry per fault, the deterministic
  interpretation layer: `physical_state_confidence`/`communication_failure`
  looked up from a **fixed table**
  (`utils/safety_fault.py::FORENSIC_FINDINGS_BY_VERIFICATION_RESULT`),
  never computed. `still_enabled` → `HIGH` confidence, `communication_
  failure=false`; `verification_comm_failure` → **`UNKNOWN`** confidence
  (never `"LOW"` -- see "Verification Result Representation" below),
  `communication_failure=true`.
- **`critical_events`** -- a *filtered view* of `timeline`
  (`scripts/export_run.py::_build_critical_events()`), never an
  independently-populated second list (which would risk silently drifting
  out of sync with `timeline`). Includes every `ERROR`/`CRITICAL`-severity
  entry, plus an explicit allowlist
  (`ALWAYS_CRITICAL_EVENT_TYPES`: `SAFETY_FAULT_RAISED`,
  `SAFETY_FAULT_ACKNOWLEDGED`, `GROUP_RUN_ABORTED_STATION_FAULT`,
  `SAFETY_MONITOR_TRIGGERED`) for events that are narratively critical
  despite being logged at `level=INFO` (an acknowledgement is a
  resolution, not a problem, but is still one of the golden forensic
  questions: "did the operator acknowledge?").
- **`run_summary`** / **`execution_state`** / **`event_log`** -- the
  underlying raw records, largely as stored; `event_log` entries also
  carry a `parsed_fields` dict (via `utils/event_format.py::
  parse_event_fields()`) for any row in the `EVENT_TYPE=...` format, so
  the reader never has to re-parse `KEY=value` text already handed to it
  structured elsewhere.
- **`measurements`** -- `row_count`, `per_channel_statistics` (min/max/avg
  voltage/current/temp), `first_sample`/`last_sample` (full rows, not
  just scalars), and a bounded `sample` -- **always present**, regardless
  of `--include-measurements` (a handful of aggregate queries, not a row
  dump). Full `rows` only with `--include-measurements`.
- **`raw_hardware_log`** -- `hardware_failure_count` and
  `unique_error_types` (from `success=0` rows) **always present**; full,
  JSON-decoded `rows` only with `--include-raw-hardware-log`
  (`command_parameters`/`response`/`additional_metadata` are stored as
  JSON text -- decoded into nested objects here rather than left as
  double-encoded strings, since that's far more readable for both a human
  and an AI).

### Verification Result Representation

Every `verification_result` field is always paired with a fixed
`verification_result_meaning` string
(`utils/safety_fault.py::VERIFICATION_RESULT_MEANINGS`), never the bare
enum token alone:
...
| Flag | Effect |
|---|---|
| `--summary-only` | `metadata`+`incident_summary`+`safety`+`forensic_findings`+`run_summary` only -- `timeline`/`critical_events`/`execution_state`/`event_log` become empty |
| `--include-measurements` | embed full `measurements.rows` |
| `--include-raw-hardware-log` | embed full `raw_hardware_log.rows` (JSON-decoded) |
| `--gzip` / `--no-gzip` | force `.json.gz` / force plain `.json` regardless of size |
| (neither) | plain `.json`, auto-switching to `.json.gz` only if the payload exceeds 10 MB |

`event_log` is included by default (not gated behind a flag) -- it's
comparatively low-volume (one row per meaningful transition, not per
sample) and is the core human-readable narrative; only `--summary-only`
suppresses it, for a fast bulk-triage pass.

### Investigation workflow

```
python scripts/export_run.py RACK01-00000001
  -> RACK01-00000001.json
  -> upload directly to Claude/Copilot: "investigate this battery test run"
```

For a quick triage pass across many runs: `--summary-only` on each,
skim `incident_summary.narrative`/`primary_failure_mode` to decide which
runs warrant a full re-export with `--include-measurements`/
`--include-raw-hardware-log` for deeper analysis.

### Test suite

New `tests/test_export_run.py` (41 cases): read-only export path
(no sequence allocation, no CSV directory, no new telemetry file
created), `telemetry_db` lookup (including a simulated month mismatch,
proving it never falls back to timestamp inference), a legacy run with
`telemetry_db IS NULL` (confirms `None`-not-`0` fault counts),
`incident_summary`/`timeline`/`critical_events`/`safety` generation,
`verification_result` → `forensic_findings` mapping (including the
never-`LOW` guarantee), `--summary-only`/`--include-measurements`/
`--include-raw-hardware-log` modes, gzip (forced and auto-threshold),
determinism (two exports of the same run are byte-identical except
`exported_at`; top-level key order is fixed), and the actual `main()` CLI
entry point in-process. Full repository suite: **568/568 passing**, no
regressions.
