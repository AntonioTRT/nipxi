...
**`run_summary` table** -- one row per run: `id` (the operator-facing Run Number), `run_id`, `test_type`, `start_time`/`end_time`/`duration_s`, `stop_reason`, `result`, and (nullable, `N/A` for Proto Test) battery-config snapshot and `capacity_ah`/`energy_wh`/`cycle_count`. Also carries Monitor Battery's voltage summary (nullable, `N/A` for every other `test_type`): `start_voltage`/`end_voltage`/`min_voltage`/`max_voltage`/`average_voltage`/`sample_count`. And a hardware-identity snapshot (nullable per role -- `dmm_*` is `NULL` when no DMM was configured): `smu_name`/`smu_resource`/`smu_model`, `dmm_name`/`dmm_resource`/`dmm_model`, `daq_name`/`daq_resource`/`daq_model`, `relay_matrix_name`/`relay_matrix_resource`/`relay_matrix_model` -- which physical instrument (by `config/devices.py` dict key, VISA resource/IP, and model) was actually connected for this run. Written via `DataStorage.start_run_summary()`/`finish_run_summary()`. See `docs/architecture.md` Section 18/20a/22.

**`event_log` table** -- fine-grained, timestamped runtime narrative (relay activated/deactivated, output enabled, measurement acquired, battery/hardware configuration traceability, ...) -- NOT a replacement for logger output; only meaningful runtime transitions are recorded. Includes one entry per connected instrument ("SMU in use: ...", "DMM in use: ...", etc.) before the first relay closes, in addition to the battery-selection sequence. Written via `DataStorage.log_event()`, read via `get_recent_events()`. See `docs/architecture.md` Section 18a/18b/22.
